...
function getLeadAttributionBadge(lead: any, sub?: any) {
  if (!lead && !sub) return null;

  const norm = (s: any) => String(s || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().trim();
  const src = norm(lead?.utm_source || sub?.utm_source || lead?.origem || sub?.origem);
  const med = norm(lead?.utm_medium || sub?.utm_medium);
  const camp = norm(lead?.utm_campaign || sub?.utm_campaign);
  ...
  const isOrganic = (organicPattern.test(src) || organicPattern.test(med) || organicPattern.test(camp) || organicPattern.test(cont) || organicPattern.test(origem))
    && !fbclid && !gclid && !fbp && !/^(cpc|cpm|ppc|paid|ads|ad|anuncio|patrocinado)$/i.test(med);
  ...
  if (src === "criar_saas" || origem === "criar_saas") {
    return { text: "Criar SaaS", class: "bg-yellow-500/15 text-yellow-300 border-yellow-500/30", textShort: "Criar SaaS", isPago: false };
  }

  // 2. Prospecção SDR
  if (src === "sdr-manual" || med === "sdr-manual" || src.includes("sdr")) {
    return { text: "Prospecção SDR", class: "bg-blue-500/15 text-blue-300 border-blue-500/30", textShort: "SDR", isPago: false };
  }

  // 3. Google Ads
  if (src.includes("google") || src.includes("gads") || gclid) {
  ...
  if (isPaidSrc || isPaidMed || isPaidCamp || !!fbclid || !!fbp) {
...
function ChatPage({ searchOverride }: { searchOverride?: ChatSearchParams } = {}) {
  ...
  const typebotByPhone = useMemo(() => {
    const m = new Map<string, any>();
    for (const l of typebotLeads as any[]) {
      const wa = String(l.whatsapp ?? "").replace(/\D+/g, "");
      if (!wa) continue;
      const keys = new Set<string>([wa]);
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex flex-wrap items-center gap-1 mt-1">
                              ...
                              {(() => {
                                 ...
                                 return (
                                   <Badge variant="outline" className={`shrink-0 h-4 px-1.5 text-[9px] font-bold uppercase ${attr.class}`}>
                                     {attr.isPago ? "💰 Tráfego Pago" : "🌱 Orgânico"}
                                   </Badge>
                                 );
                               })()}
                ...
                <div className="flex min-w-0 items-center gap-2 md:gap-4">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-0.5">
                      ...
                      {(() => {
                        const lead = findLeadForConv(active.contact_wa_id);
                        const d = String(active.contact_wa_id ?? "").replace(/\D+/g, "");
                        const sub = typebotByPhone.get(d) || (d.length >= 8 ? typebotByPhone.get(d.slice(-8)) : null);
                        const attr = getLeadAttributionBadge(lead, sub);
                        const badgeObj = attr ?? { text: "Orgânico", class: "bg-amber-500/10 text-amber-400 border-amber-500/40" };
                        const campaign = (lead as any)?.utm_campaign || sub?.utm_campaign ? ` · Campanha: ${(lead as any)?.utm_campaign || sub?.utm_campaign}` : "";
                        const isLinkInBio = /link_?in_?bio|link_?bio|ig_?bio|bio_?instagram|instagram_?bio|\bbio\b/i.test(String((lead as any)?.utm_content || sub?.utm_content || "")) || /link_?in_?bio|link_?bio|ig_?bio|bio_?instagram|instagram_?bio|\bbio\b/i.test(String((lead as any)?.utm_source || sub?.utm_source || ""));
                        const adContent = ((lead as any)?.utm_content || sub?.utm_content) && !isLinkInBio ? ` · Anúncio: ${(lead as any)?.utm_content || sub?.utm_content}` : "";
  ...
  return (
    <div className="relative group/stk inline-block">
      <img
        ...
        role="button"
        tabIndex={0}
        className={`mb-2 block cursor-zoom-in object-contain transition hover:opacity-95 ${
          isSticker
            ? "max-h-44 max-w-44 bg-transparent p-0 shadow-none border-0"
            : "max-h-[420px] max-w-full rounded-2xl border border-chat-line"
        }`}
      />
      {isSticker && (
        <button
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          variant="ghost"
          size="icon"
          className="h-10 w-10 md:h-12 md:w-12 shrink-0 rounded-2xl text-muted-foreground hover:bg-chat-soft hover:text-chat-accent transition-all"
          title="Figurinhas (Stickers)"
        >
          <Sticker className="h-5 w-5" />
        </Button>
      </PopoverTrigger>
      <PopoverContent align="start" side="top" className="w-80 md:w-96 rounded-2xl border border-chat-line bg-popover/95 backdrop-blur-md p-3.5 shadow-2xl">
        <div className="flex items-center justify-between gap-2 border-b border-chat-line/60 pb-3 mb-3">
          <div className="flex items-center gap-1 bg-chat-panel/80 p-1 rounded-xl border border-chat-line/50 flex-1">
            <button
              type="button"
              onClick={() => setActiveTab("all")}
              className={`flex-1 flex items-center justify-center gap-1.5 px-2.5 py-1.5 text-xs font-semibold rounded-lg transition-all ${
                activeTab === "all"
                  ? "bg-chat-accent text-chat-accent-foreground shadow-sm font-bold"
                  : "text-muted-foreground hover:text-foreground hover:bg-white/5"
              }`}
            >
              <Images className="h-3.5 w-3.5" />
              <span>Galeria</span>
              <span className="text-[10px] opacity-75">({stickers.length})</span>
            </button>
            <button
              type="button"
            ...
            >
              ...
              <span>Favoritas</span>
              <span className="text-[10px] opacity-75">({stickers.filter((s) => s.isFavorite).length})</span>
            </button>
          </div>
          ...
          <Button
            ...
            className="h-9 text-xs gap-1.5 rounded-xl bg-chat-accent text-chat-accent-foreground hover:bg-chat-accent/90 shadow-sm font-semibold shrink-0"
          >
        ...
        <div className="max-h-64 overflow-y-auto pr-1 scrollbar-fancy">
          {uploading && (
            <div className="flex items-center justify-center gap-2 py-6 text-xs text-muted-foreground bg-chat-panel/40 rounded-xl mb-2 border border-chat-line/40">
              <Loader2 className="h-4 w-4 animate-spin text-chat-accent" />
              <span>Formatando figurinha 512x512...</span>
            </div>
          )}

          {filteredStickers.length === 0 ? (
            <div className="py-10 flex flex-col items-center justify-center text-center text-xs text-muted-foreground border border-dashed border-chat-line/60 rounded-xl bg-chat-panel/20 p-4">
              ...
              <p className="text-[11px] text-muted-foreground/70 mt-1">
                Clique em "+ Criar" para adicionar novas figurinhas!
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-4 gap-2">
              {filteredStickers.map((stk) => (
                <div
                  ...
                  className="group relative flex aspect-square cursor-pointer items-center justify-center rounded-xl p-1.5 transition-all hover:scale-110 hover:bg-white/10"
                >
                  <img
                    src={stk.url}
                  ...
                  />
                  <div className="absolute top-1 right-1 flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity bg-black/75 rounded-lg p-0.5 backdrop-blur shadow">
                    <button
                      type="button"
