...
import { searchLeadsForShowup } from "@/lib/ht-api.functions";
...
export function ShowUpDialog({
  ...
  const searchFn = useServerFn(searchLeadsForShowup);
  ...
  async function doSearch(term: string) {
    ...
    try {
      ...
      // 1) Busca server-side (ht_quiz_submissions + crm_leads via supabaseAdmin)
      try {
        const serverRes = await searchFn({ data: { query: t } });
        for (const r of (serverRes?.leads ?? []) as QuizLead[]) {
          const key = r.email || r.whatsapp || r.id;
          if (key && !seen.has(key)) { seen.add(key); all.push(r); }
        }
      } catch {}

      // 2) Busca no quiz externo
      try {
        let qExt = getQuizSb()
          .from("leads")
