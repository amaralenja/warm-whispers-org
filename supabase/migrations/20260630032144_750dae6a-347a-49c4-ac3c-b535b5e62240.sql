...
CREATE OR REPLACE FUNCTION public.get_vendor_stats(_utm text, _from date DEFAULT NULL, _to date DEFAULT NULL)
RETURNS jsonb
LANGUAGE plpgsql
...
BEGIN
  ...
  period AS (
    SELECT * FROM sales WHERE sale_date BETWEEN d_from AND d_to
  ),
  my_period AS (
    SELECT * FROM period WHERE utm = v_utm
  ),
  rank_all AS (
    SELECT
      vd.utm,
  ...
  SELECT jsonb_build_object(
    ...
    'metaPct', (
      SELECT CASE WHEN vi.meta > 0
        THEN LEAST(100, (COALESCE((SELECT SUM(ticket) FROM my_period), 0) / vi.meta) * 100)
        ELSE 0 END
      FROM vendor_info vi
    ),
    'faltaMeta', (
      SELECT CASE WHEN vi.meta > 0
        THEN GREATEST(0, vi.meta - COALESCE((SELECT SUM(ticket) FROM my_period), 0))
        ELSE 0 END
      FROM vendor_info vi
    ),
...
GRANT EXECUTE ON FUNCTION public.get_vendor_stats(text, date, date) TO anon, authenticated;
