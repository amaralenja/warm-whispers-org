...
DECLARE
  ...
  mes_passado_inicio date := (date_trunc('month', now()) - interval '1 month')::date;
  mes_passado_fim date := (date_trunc('month', now()) - interval '1 day')::date;
...
BEGIN
  ...
  filtered_passado AS (
    SELECT * FROM sales WHERE sale_date BETWEEN mes_passado_inicio AND mes_passado_fim
  ),
  vendor_mes AS (
    SELECT
    ...
    LEFT JOIN filtered_mes f ON f.utm = upper(trim(vd.utm))
  ...
  vendor_passado AS (
    SELECT
      ...
      COUNT(f.*)::int AS vendas
    FROM public.vendedores vd
    LEFT JOIN filtered_passado f ON f.utm = upper(trim(vd.utm))
    WHERE COALESCE(vd.ativo,true) = true
    GROUP BY vd.utm, vd.nome, vd.expert, vd.genero, vd.foto_url
  )
  SELECT jsonb_build_object(
    ...
    'rainha', (
      SELECT to_jsonb(t) FROM (
        SELECT utm, nome, expert, foto_url AS "fotoUrl", faturamento, vendas, 0::numeric AS meta
        FROM vendor_passado
        WHERE genero = 'F' AND vendas > 0
        ORDER BY faturamento DESC LIMIT 1
      ) t
    ),
    'metaLobo', meta_lobo,
