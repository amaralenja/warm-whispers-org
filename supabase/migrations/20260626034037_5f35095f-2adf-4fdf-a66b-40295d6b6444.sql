...
CREATE OR REPLACE FUNCTION public.get_ranking_tv_stats(_from date DEFAULT NULL::date, _to date DEFAULT NULL::date)
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
  ...
  goal_hits AS (
    SELECT
      posicao, nome, utm, expert, meta, faturamento, vendas,
  ...
  SELECT jsonb_build_object(
    ...
    'metaLogs', COALESCE((
      SELECT jsonb_agg(
        jsonb_build_object(
          'utm', g.utm, 'nome', g.nome, 'expert', g.expert,
          'meta', g.meta, 'faturamento', g.faturamento, 'vendas', g.vendas, 'batida', g.batida
