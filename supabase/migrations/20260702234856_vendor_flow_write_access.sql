-- Permite que sessões de vendedor (auth custom por código) criem/editem/importem fluxos
-- sem depender de um usuário Supabase autenticado. Todas as funções validam o vendedor
-- e restringem a operação aos workspaces liberados para ele.
...
BEGIN
  ...
  INSERT INTO public.wa_flows (
    nome, descricao, operacao_id, folder, ativo, entry_node_id, nodes, edges, created_by
  ) VALUES (
...
BEGIN
  ...
  SELECT
    _flow_id,
    coalesce(t->>'tipo', 'manual'),
    nullif(t->>'valor', ''),
    coalesce(nullif(t->>'match_mode', ''), 'contains'),
    nullif(t->>'channel_id', ''),
...
BEGIN
  ...
  RETURN QUERY
  SELECT t.*
  FROM public.wa_flow_triggers t
  JOIN public.wa_flows f ON f.id = t.flow_id
  WHERE t.flow_id = _flow_id
    AND (
      f.operacao_id IS NULL OR EXISTS (
        SELECT 1 FROM unnest(allowed) a
        WHERE public._vendor_norm(f.operacao_id) = public._vendor_norm(a)
